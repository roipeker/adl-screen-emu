...
public class ScreenEmulator extends EventDispatcher {
    ...
    private var _device:ScreenDefinition;
    ...
    public function get screenDPI():Number {
        return _device ? _device.ppi : Capabilities.screenDPI;
    }
    ...
    private function onStageOrientationChange(event:StageOrientationEvent):void {
        ...
        if (!_device) {
            trace("[ ScreenEmulator ] can't change orientation without an emulation configuration.");
            return;
        }
     ...
     * @param screen
     */
    public function emulate(screen:ScreenDefinition) {
        if( _device == screen ) {
            return ;
        }
        _device = screen;
        calculateDensity(_device.ppi);
...
//        _stage.setOrientation(_eventOrientation);
    ...
    public function get device():ScreenDefinition {
        return _device;
    }
}
}